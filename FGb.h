...
{
    FGB(init_urgent)
    (4, 2, "DRLDRL", 100000, 0); /* Do not change the following parameters
    ...
    FGB(init)
    (1, 1, 0, log_output); /* do not change */
    FGB(reset_coeffs)
    (1); /* We compute in Q[x1,x2,x3,x4,x5,x6] */
}
...
struct FGbRunner {
    ...
    std::vector<Dpol> convertInput(const std::vector<P>& input)
    {
        ...
        for (typename std::vector<P>::size_type i = 0; i < input.size(); ++i) {
            ...
            for (const auto& term : input[i]) {
                ...
                FGB(set_coeff_gmp)
                (input_basis[i], j, term.c().get_mpz_t());
                ++j;
            }
            FGB(full_sort_poly2)
            (input_basis[i]);
    ...
    std::vector<P> convertOutput(const std::vector<Dpol>& output_basis)
    {
        ...
        for (std::vector<Dpol>::size_type i = 0; i < output_basis.size(); ++i) {
            ...
            FGB(export_poly_INT_gmp2)
            (M::VAR_COUNT, term_count, coefficients.data(), exponents.data(), output_basis[i]);
    ...
    std::vector<P> fgb(const std::vector<P>& input)
    {
        ...
        FGB(PowerSet)
        (M::VAR_COUNT, 0, var_names_as_char.data());
        threads_FGb(16);
